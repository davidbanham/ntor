...
exports.html = function(req, res, files, next, dir, showUp, icons){
  fs.readFile(__dirname + '/../public/directory.html', 'utf8', function(err, str){
    ...
    fs.readFile(__dirname + '/../public/style.css', 'utf8', function(err, style){
      ...
      str = str
        .replace('{style}', style)
        .replace('{files}', html(files, dir, icons, req))
...
function html(files, dir, useIcons, req) {
  return '<ul id="files">' + files.map(function(file){
    ...
    return '<li>'
			...
			+ req.headers.host
      + join(dir, file)
			+ '?sessu='+encodeURIComponent(req.ntor)
      + '" class="'
      + classes.join(' ') + '"'
      + ' title="' + file + '">'
      + icon + file + '</a>'
      + '<form method="POST" action="/queue/item"><input type="hidden" name="path" value="'+join(dir.replace('/incoming', ''), file)+'"><input type="submit" value="Queue"></form>'
      + '</li>'
